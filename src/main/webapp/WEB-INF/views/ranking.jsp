...
<head>
    ...
    function start1() {
        ...
        $.ajax({
            ...
            success: function (response) {
                $('#rank_post_list_title').html(response.catedetailname+' 강의랭킹 게시판');
            }
        });
        console.log($('#tt').html());
    }
    ...
    //                 var rankBoardList = $('<div class="rankboard_list_box"><div class="rankboard_list_number"><h1>'+(Number(i)+1)+'</h1></div><div class="rankboard_list_inner"><a href="/ranking?postnum='+response[i].postnum+'"><h3>'+response[i].posttitle+'</h3></a><p>'+response[i].author+' - '+response[i].classtitle+'</p></div></div>');
                        ...
                        <div id="rank_post_list_area">
                            <div id="rank_post_list">
                                ...
                                <div id="postsearch_area">
                                    <button id="templist_btn"><a href="/rankingtempsave?catedetailcode=${catedetailcode}&page=1&memberid=${member.id}">임시저장 목록</a></button>
                                    <button id="rankwrite_btn"><a href="/rankingwrite?catedetailcode=${catedetailcode}">새글</a></button>
                                    ...
                                    <c:forEach items="${ postlist }" var="list" >
                                        <div class="rank_post_box">
                                            <div class="rank_post_box2">
                                                <div class="rank_post_thumbnail">
                                                    <img class="rank_post_thumbnail_inner" src="css/images/${list.thumbnail}">
                                                </div>
                                ...
                                <div id="paging">
                                    <c:if test="${paging.previousPageStart != -1}">
                                        <div class="paging_btn"><a href="/rankingsearch?catedetailcode=${catedetailcode}&page=${paging.previousPageStart}&postsearch=${postsearch}">이전</a></div>
                                    </c:if>
                                    <c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="num">
                                        <div class="paging_btn"><a href="/rankingsearch?catedetailcode=${catedetailcode}&page=${num}&postsearch=${postsearch}">${num}</a></div>
                                    </c:forEach>
                                    <c:if test="${paging.nextPageStart != -1}">
                                        <div class="paging_btn"><a href="/rankingsearch?catedetailcode=${catedetailcode}&page=${paging.nextPageStart}&postsearch=${postsearch}">다음</a></div>
                                    </c:if>
                                </div>
                                <div class="inner_space"></div>
                            </div>
