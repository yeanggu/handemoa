 ...
 */ 
	...
	$(document).on('click', '#search_btn', (function () {
		...
		location.href = '/adminpostreport/search?divisioncode=' + division + '&currentpage=1' + '&search=' + search + '&searchtxt=' + searchtxt;	
	}));
	...
	$(document).on('click', '#modal_confirm_ok_btn', (function () {
		...
		$.ajax({
			url: "/adminpostreport/delete",
			data: { 'postnums': postnums },
			type: 'post',
			dataType: 'json',
			success: function (result) {
				console.log(result);
				//location.reload();
				location.replace("/adminpostreport/division?divisioncode=0&currentpage=1");
			}
		});
	})); //삭제 확인 end
