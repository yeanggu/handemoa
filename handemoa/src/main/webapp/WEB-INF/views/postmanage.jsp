                ...
                <div id="admin_content">
                    ...
                    <div id="postmanage_btn_area">
	                    <div id="postmanage_btn_box">
	                        <input type="button" id="postmanage_btn" style="cursor:pointer;" onclick="historyValue();" value="삭 제">
	                    </div>
                    ...
                    <div class="paginate">
				        <div class="paging">
				            <a class="direction_prev" href="postmanage?currentPage=1&postType=${pagedto.postType }${pagedto.searchTypeKeyword}"> &lt;&lt; </a> 
				            <c:if test="${pagedto.hasPreviousPage == true}">
				            	<a class="direction_prev" href="postmanage?currentPage=${pagedto.currentPage-1}&postType=${pagedto.postType }${pagedto.searchTypeKeyword}"> &lt;</a>
				            </c:if>
				            <c:if test="${pagedto.hasPreviousPage == false}">
				            	<a class="direction_prev" href="postmanage?currentPage=${pagedto.currentPage}&postType=${pagedto.postType }${pagedto.searchTypeKeyword}"> &lt;</a>
				            </c:if>
				            
				            <c:forEach begin="${pagedto.firstPage}" end="${pagedto.lastPage}" var="idx">
				                <a href="postmanage?currentPage=${idx}&postType=${pagedto.postType }${pagedto.searchTypeKeyword}" id="idx"><c:out value="${idx}" /></a>
				            </c:forEach>
				            
				            <c:if test="${pagedto.hasNextPage == true}">
				            	<a class="direction_next" href="postmanage?currentPage=${pagedto.currentPage+1}&postType=${pagedto.postType }${pagedto.searchTypeKeyword}"> &gt;</a>
				            </c:if>
				            <c:if test="${pagedto.hasNextPage == false}">
				            	<a class="direction_next" href="postmanage?currentPage=${pagedto.currentPage}&postType=${pagedto.postType }${pagedto.searchTypeKeyword}"> &gt;</a>
				            </c:if>
				            
				            <a class="direction_next" href="postmanage?currentPage=${pagedto.lastPage}&postType=${pagedto.postType }${pagedto.searchTypeKeyword}"> &gt;&gt; </a>
				        </div>
				    </div>
